...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('log/signal_control.log'),
    ...
    def __init__(self,
        ...
        try:
            ...
            if not self._cap.isOpened():
                logger.error(f'Critical error: Couldn\'t find a camera linked to input index {cam_idx}.')
...
def camera_worker(queue: Queue, flag: threading.Event, cam_idx: int, width: int, height: int):
    try:
...
def main():
    ...
    try:
        ...
        cam_worker = threading.Thread(
            ...
            args=(cam_queue, flag, args.camIndex, args.width, args.height),
            daemon=True
        )
        cam_worker.start()
